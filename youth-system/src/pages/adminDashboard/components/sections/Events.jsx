...
export default function Events() {
  ...
  return (
    <div className="border-l-2 rounded p-5 m-5 flex flex-col h-screen">
      <div className="flex items-center justify-between gap-5 border-b border-gray-200 p-3 sticky top-0 bg-white z-10">
        <h3 className="text-xl border-b-3 border-gray-400 p-2 rounded">
          Events
        </h3>
      ...
      >
        {loadEvents ? (
          ...
          events.map((ev) => (
            <EventItem key={ev._id} event={ev} setEvents={setEvents} />
          ))
        )}
      </div>
    </div>
